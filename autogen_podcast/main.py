import os
from autogen_podcast.modules import llm, log, orchestrator
...
def main():
    ...
    if True: #Create outline
        ...
        with open('./output/outline', "w") as f:
            f.write(outline)
        ...
        with open('./output/outline', 'r') as f:
            outline = f.read()
    ...
    if True: #Create script
        ...
        for i, section in enumerate(sections[starting_script+1:]):
            ...
            with open(f'./output/script_{i}', "w") as f:
                f.write(script)
